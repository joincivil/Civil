...
class RevisionComponent extends React.Component<RevisionProps & DispatchProp<any>> {
  public renderJsonSection(): JSX.Element {
    if (!this.props.revisionJson) {
      return <Tr><Td><button onClick={this.onClick}>get revision</button></Td></Tr>;
    }
    const revisionJson = this.props.revisionJson;
    return (<>
      <Tr><Td>title: {revisionJson.title}</Td></Tr>
      <Tr><Td>slug: {revisionJson.slug}</Td></Tr>
      <Tr><Td>description: {revisionJson.description}</Td></Tr>
      {revisionJson.images.length && revisionJson.images.map((item: any) => {
        return (<>
          <Tr>
            <Td>height: {item.h}</Td>
            <Td>width: {item.w}</Td>
      ...
      <Tr><Td>opinion: {`${revisionJson.opinion}`}</Td></Tr>
      <Tr><Td>original publish date: {revisionJson.originalPublishDate}</Td></Tr>
      <Tr><Td>primary tag: {revisionJson.primaryTag}</Td></Tr>
      <Tr><Td>revision content hash: {revisionJson.revisionContentHash}</Td></Tr>
      <Tr><Td>revision content url: {revisionJson.revisionContentUrl}</Td></Tr>
      <Tr><Td>revision date: {revisionJson.revisionDate}</Td></Tr>
      <Tr><Td>tags: {revisionJson.tags.join(", ")}</Td></Tr>
    </>);
  }
  public render(): JSX.Element {
    return (<Table>
        ...
        <tbody>
          <Tr><Td>revision url: {this.props.revision.uri}</Td></Tr>
          <Tr><Td>hash:{this.props.revision.contentHash}</Td></Tr>
          <Tr>
            <Td>block number: {this.props.revision.blockNumber}</Td>
            <Td>time stamp: {this.props.revision.timestamp.toDateString()}</Td>
          </Tr>
          <Tr><Td>transaction hash: {this.props.revision.transactionHash}</Td></Tr>
          {this.renderJsonSection()}
        </tbody>
    </Table>);
  }
  private onClick = (): void => {
    this.props.dispatch!(fetchRevisionJson(
      this.props.revision.uri,
      this.props.revision.contentId,
      this.props.revision.revisionId!,
    ));
  }
}
