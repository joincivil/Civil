...
module.exports = (deployer: any, network: string, accounts: string[]) => {
  ...
  deployer.then(async () => {
    if (network === "ganache") {
      ...
      await token.approve(civilTCR.address, config.paramDefaults.minDeposit * 10, { from: accounts[5] });
      await token.approve(civilTCR.address, config.paramDefaults.minDeposit * 10, { from: accounts[6] });
