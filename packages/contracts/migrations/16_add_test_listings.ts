module.exports = (deployer: any, network: string, accounts: string[]) => {
  ...
  deployer.then(async () => {
    if (network === "ganache") {
      ...
      await token.approve(civilTCR.address, 1000000000000000000000, { from: accounts[5] });
      await token.approve(civilTCR.address, 1000000000000000000000, { from: accounts[6] });

      const newsroom1 = await Newsroom.new("Fake News 1000", "http://fakecharter.uri", web3.sha3(), {
        from: accounts[5],
      });
      await civilTCR.apply(newsroom1.address, 100000000000000000000, "", { from: accounts[5] });

      const newsroom2 = await Newsroom.new("The Ultra News 2000", "https://fakecharter.com", web3.sha3(), {
        from: accounts[5],
      });
      await civilTCR.apply(newsroom2.address, 100000000000000000000, "", { from: accounts[5] });
