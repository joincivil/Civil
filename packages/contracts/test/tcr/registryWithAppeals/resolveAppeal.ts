...
contract("Registry With Appeals", accounts => {
  describe("Function: resolveAppeal", () => {
    ...
    it("should fail if apply stage in progress without a challenge", async () => {
      ...
      expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.rejectedWith(
        REVERTED,
        "should not have been able to resolve appeal on application without challenge",
      );
    });
    ...
    it("should fail if request appeal is over without one requested", async () => {
      ...
      await expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.rejectedWith(
        REVERTED,
    ...
    it("should fail if request appeal is over and one is requested", async () => {
      ...
      await expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.rejectedWith(
        REVERTED,
        "should not have been able to update status before appeal is over",
      );
    });
    ...
    it("should succeed if appeal is requested and that phase ends without one being granted ", async () => {
      ...
      await expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.fulfilled(
        "should have been able to resolve appeal after appeal judgment phase ends",
      );
    });
    ...
    it("should fail if appealChallenge is over", async () => {
      ...
      await expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.rejectedWith(
        REVERTED,
        "should not have been able to resolve appeal after appeal challenge",
      );
    });
    ...
    it("should fail if appealChallenge is in progress", async () => {
      ...
      await expect(registry.resolveAppeal(newsroomAddress)).to.eventually.be.rejectedWith(
        REVERTED,
        "should not have been able to update status since granted appeal was challenged",
      );
    });
  });
});
