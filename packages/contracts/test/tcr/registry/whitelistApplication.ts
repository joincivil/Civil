...
contract("Registry", accounts => {
  describe("Function: whitelistApplication", () => {
    ...
    it("should not whitelist a listing that is still pending an application", async () => {
      ...
      await expect(registry.whitelistApplication(listing22, { from: applicant })).to.be.rejectedWith(
        REVERTED,
    ...
    it("should not whitelist a listing that is currently being challenged", async () => {
      ...
      await expect(registry.whitelistApplication(listing23)).to.eventually.be.rejectedWith(
        REVERTED,
        "Listing should not have been whitelisted",
      );
    });

    it("should not whitelist a listing that was challenged", async () => {
    ...
    it("should not be possible to add a listing to the whitelist just by calling whitelistApplication", async () => {
      await expect(registry.whitelistApplication(listing25, { from: applicant })).to.eventually.be.rejectedWith(
        REVERTED,
        "Listing should not have been whitelisted",
      );
    });
    ...
    it(
      "should not be possible to add a listing to the whitelist just " +
        "by calling whitelistApplication after it has been previously removed",
      async () => {
        ...
        await expect(registry.whitelistApplication(listing26, { from: applicant })).to.eventually.be.rejectedWith(
          REVERTED,
          "Listing should not have been whitelisted",
        );
      },
